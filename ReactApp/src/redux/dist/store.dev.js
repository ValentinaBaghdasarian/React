...
var store = {
  ...
  getState: function getState() {
    // վերադարձնում է սթեյթը
    return this._state;
  },
  subscribe: function subscribe(observer) {
    // index.js ֆայլում rerenderEntireTree կոմպոնենտը կանչելու համար ստեղծվել է subscribe ֆունկցիան որը state.js ֆայլում նկարագրված rerenderEntireTree ֆունկցիային տալիս է index.js ֆայլում նկարագրված rerenderEntireTree ֆունկցիան , որը կանչվում է index.js ֆայլում
  ...
  dispatch: function dispatch(action) {
    ...
    this._rerenderEntireTree(this._state); // this._state.dialog = dialogReducer(this._state.dialog, action);


    this._state.dialog = (0, _dialogReducer["default"])(this._state.dialog, action);
